...
public class Rat0 extends Robot {
	...
	public void run() {
		...
		while (step(timeStep) != -1) {
			...
			// LEFT WALL FOLLOWING IMPLEMENTATION
			// the basic rule is that we always have the wall on our left hand and and we
			// dont stop touching it

			// distance[2] is the right distance sensor of the robot, if it is less than 80
			// it dosent see a wall
			boolean wallLeft = (distance[2] > 80);
			boolean bumpLeftWall = (distance[2] > 1000);
			...
			if (wallFront) {
				...
				if (wallLeft) {
					...
					// if the robot bumps the left wall then move away from it a little
					if (bumpLeftWall) {
						leftSpeed = (-3) * maxSpeed / 4;
						rightSpeed = -maxSpeed;
						System.out.println("Bumbed the Left Wall --> Move Away");
					} else {
						...
						if (distance[2] < 200) {
							...
							// if the robot is too far from the left wall then come a little closer to it
							leftSpeed = -maxSpeed;
							rightSpeed = (-3.5) * maxSpeed / 4;
							System.out.println("Too Far from Left Wall --> Close Distance");
						}
					}
				} else {
					// if there is not a wall in left , if left you have a gap Turn Left
					leftSpeed = -maxSpeed;
					rightSpeed = -maxSpeed / 8;
					System.out.println("Gap on the Left --> Turn Left");
				}
			}
			...
			// if (distance[3] > 1800 || distance[4] > 1800) {
			// if (!turn) {
			// turn = true;
			// right = r.nextBoolean();
			// }
